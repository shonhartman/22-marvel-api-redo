/*
  Character Module Configuration Function
  --------------------------

  This is where you pass in config time options to Providers.
  For example, this is where you can configure $stateProvider
  to set up your routing.

  STEP 1: Use $stateProvider.state to add a single route called
          characters that points to /characters/:name

          It should use the CharactersController and call it charCtrl.

          It should use the ./character.html template. Don't forget,
          we use require() to include templates!
*/
function config($stateProvider) {
  $stateProvider
  .state('characters', {
    url: '/characters/:name',
    controller: "CharacterController as charCtrl",
    template: require('./character.html')

  });
}

export default config;
